  ...
  <section class="productos py-4">
    ...
    <div class="row my-5 mx-0 justify-content-center">
      <div v-for="consola in datos" v-bind:key="consola.id" class="bg-light card mx-1 my-1 mx-md-5" style="min-width: 200px;max-width: 20vw">
        ...
        <img class="card-img-top bg-white border-bottom" v-bind:src='consola.imagen'>
        ...
        <div class="card-body d-flex row justify-content-center">
          <h4 class="card-title font-weight-bold">{{consola.precio}} €</h4>
...
.card-header {
  @include header-card(color(secundario))
}

.card-title {
  @include title-card(color(secundario))
}

section {
  background-image: $fondofeo2
}
